...
with open("apps.json") as f:
    APP_MAP = json.load(f)  # {"app1": "frontend-app1", ...}
...
def update_excel_local(data):
    ...
    with lock:
        ...
        headers = ["Application", "Version", "Environment", "JobType", "Branch", "Repo", "Comments", "Status", "Timestamp"]
...
def trigger_github_actions(app_name, version, environment, job_type, branch):
    ...
    if environment not in ENVIRONMENTS:
        raise ValueError(f"Branch/environment '{environment}' not found in the repo.")
...
def index():
    if request.method == "POST":
        ...
        try:
            ...
            update_excel_local([app_name, version, environment, job_type, branch, repo_name, comments, success_msg])
            ...
            update_excel_local([app_name, version, environment, job_type, branch, repo_name, comments, error_msg])
